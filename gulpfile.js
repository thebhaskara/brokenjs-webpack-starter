...
gulp.task('add', function (cb) {
    ...
    else if (argv.style) dest = 'scss';
    else throw "specify --component or --control or --page or --utility or --binder or --style";
    var name = argv.component || argv.control || argv.page || argv.utility || argv.binder || argv.style;
    name = kebabCase(name);
...
${upperCamelCase(name)}, `);
        ...
        fs.writeFileSync(dir + '.scss', '');
        var indexPath = [destDir, 'index.scss'].join('/');
        var indexContent = fs.readFileSync(indexPath).toString();
        indexContent = indexContent.replace(importComment, `${importComment}
@import './${name}';`);
        fs.writeFileSync(indexPath, indexContent);
